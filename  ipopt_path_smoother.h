...
class PathSmoothIpoptInterface : public Ipopt::TNLP {
 public:
  /** default constructor */
  PathSmoothIpoptInterface(const behavior_planning::MpInputMsgs& mp_input_msgs);

  void get_path_points(std::vector<planning::PathPoint>* origin_path_points,
                       std::vector<planning::PathPoint>* smooth_path_points);

  /** default destructor */
  virtual ~PathSmoothIpoptInterface() = default;
 ...
 private:
   ...
   *
   */
  PathSmoothIpoptInterface(const PathSmoothIpoptInterface&);
  PathSmoothIpoptInterface& operator=(const PathSmoothIpoptInterface&);
 ...
 private:
  TrajectoryPoint ego_traj_point_;
  std::shared_ptr<motion_planning::data_class::GridMap> grid_map_ptr_;
  std::unordered_map<std::string, Obstacle> interest_obstacles_;

 private:
  int num_of_points_;
  double ds_ = 0.0;
  std::vector<planning::PathPoint> reference_line_;
  std::vector<planning::PathPoint> matched_reference_points_;
  std::vector<std::pair<double, double>> x_loc_bounds_;
  std::vector<std::pair<double, double>> y_loc_bounds_;
  
  std::vector<planning::SLPoint> origin_sl_points_;
  std::vector<planning::PathPoint> origin_path_points_;
  std::vector<planning::PathPoint> smooth_path_points_;
};
